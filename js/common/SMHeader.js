/**
 * @providesModule SMHeader
 * @flow
 */

'use strict';

var SMColors = require('./SMColors');
...
class SMHeaderAndroid extends React.Component {
...
class SMHeaderIOS extends React.Component {
  static height: number;
  props: Props;

  render() {
    const {leftItem, title, rightItem, foreground} = this.props;
    const titleColor = foreground === 'dark' ? SMColors.darkText : 'white';
    const itemsColor = foreground === 'dark' ? SMColors.lightText : 'white';
...
const Header = Platform.OS === 'ios' ? SMHeaderIOS : SMHeaderAndroid;
